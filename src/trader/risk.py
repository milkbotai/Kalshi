...
class CircuitBreaker:
    ...
    def __init__(
        ...
        Args:
            max_daily_loss: Maximum daily loss in dollars before pause (5% of bankroll)
    ...
    def check_daily_loss_limit(
        ...
        if total_pnl < -self.max_daily_loss:
            if not self._paused:
                self._paused = True
                self._pause_reason = (
                    f"Daily loss ${abs(total_pnl):.2f} exceeds limit ${self.max_daily_loss:.2f}"
                )
    ...
    def track_order_rejects(self, reject_timestamp: float) -> int:
        ...
        self._reject_timestamps = [ts for ts in self._reject_timestamps if ts >= window_start]
